...
public:
  ...
  : Node("thermal_map_builder"), _grid(std::move(grid))
  {
    ...
    _thermal_sub = create_subscription<sensor_msgs::msg::Temperature>("/thermal_reading",
        rclcpp::QoS(10),
                                                                          std::bind(
        &ThermalMapBuilder::ThermalCallback, this, std::placeholders::_1));
    _ogrid_sub = create_subscription<nav_msgs::msg::OccupancyGrid>("/map", map_qos,
        std::bind(&ThermalMapBuilder::MapCallback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(get_logger(), "Thermal Map Builder ready - publishing /thermal_map at %.1f Hz",
        rate);
  }
