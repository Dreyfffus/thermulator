...
// Processing class for thermal map generation
class ThermalGrid final {
  ...
  public:
    ...
    // Dynamically resize in case of /map resize. Recenters the map based on coordinate
    void Resize(const nav_msgs::msg::MapMetaData &new_info);

    // nav2_msgs::msg::OccupancyGrid conversion compatibility
    std::vector<int8_t> ToOccupancyData(
        double cold_thresh, double hot_thresh,
  ...
  private:
    ...
    // Makes the gradient radius for the generated data.
