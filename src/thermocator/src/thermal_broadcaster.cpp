...
public:
  ...
  : Node("thermal_broadcaster"), _rng(std::random_device{}())
  {
    ...
    for (size_t i = 0; i < cx.size(); ++i) {
      ...
      RCLCPP_INFO(get_logger(), "Zone %zu — center (%.2f, %.2f)  peak %.1f°C  sigma %.2fm", i,
          cx[i], cy[i], peaks[i], sigs[i]);
    }
    ...
    _temp_pub = create_publisher<sensor_msgs::msg::Temperature>("/thermal_reading",
        rclcpp::QoS(10));
...
void ThermalBroadcaster::TimerCallback()
{
  ...
  try {
    ...
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000, "TF Lookup failed (%s) - skipping tick",
        e.what());
    return;
  }
